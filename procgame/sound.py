...
class PinboxSoundController(object):
	"""Wrapper for pygame sound."""
	
	enabled = True
	
	def __init__(self, delegate):
		super(PinboxSoundController, self).__init__()
		self.logger = logging.getLogger("game.PinboxSound")

	def play_music(self, key, loops=0, start_time=0.0):
		"""Start playing music at the given *key*."""
		if not self.enabled: return
		base.messenger.send("play_music", [key,loops,start_time])

	def stop_music(self):
		"""Stop the currently-playing music."""
		if not self.enabled: return
		base.messenger.send("stop_music")

	def fadeout_music(self, time_ms = 450):
		""" TODO: UNSUPPORTED IN PANDA3D. WE MUST DO THIS IN SOFTWARE """
		if not self.enabled: return
		base.messenger.send("fadeout_music", [time_ms])

	def load_music(self, key):
		""" """
		if not self.enabled: return
		base.messenger.send("load_music", [key])
	...
	def register_sound(self, key, sound_file):
		...
		if not self.enabled: return
		if os.path.isfile(sound_file):
			base.messenger.send("register_sound", [key, sound_file])
		else:
			self.logger.error("Sound registration error: file %s does not exist!", sound_file)
	...
	def register_music(self, key, music_file):
		...
		if not self.enabled: return
		if os.path.isfile(music_file):
			base.messenger.send("register_music", [key, music_file])
		else:
			self.logger.error("Music registration error: file %s does not exist!", music_file)

	def play(self,key, loops=0, max_time=0, fade_ms=0, fade_music=False):
		""" """
		if not self.enabled: return
		base.messenger.send("play", [key, loops, max_time, fade_ms, fade_music])
		
	def play_delayed(self, key, loops, delay,callback=None):
		if not self.enabled: return
		base.messenger.send("play_delayed", [key, loops, delay,callback])
		
	def stop_delayed(self, key):
		if not self.enabled: return
		base.messenger.send("stop_delayed", [key])

	def play_voice(self,key, loops=0, max_time=0, fade_ms=0, fade_music=False):
		""" """
		if not self.enabled: return 0
		base.messenger.send("play_voice", [key, loops, max_time, fade_ms, fade_music])

	def stop(self,key, loops=0, max_time=0, fade_ms=0):
		""" """
		if not self.enabled: return
		base.messenger.send("stop", [key, loops, max_time, fade_ms])

	def volume_up(self):
		""" """
		if not self.enabled: return
		base.messenger.send("volume_up")

	def volume_down(self):
		""" """
		if not self.enabled: return
		base.messenger.send("volume_down")

	def set_volume(self, new_volume):
		""" """
		if not self.enabled: return
		base.messenger.send("set_volume", [new_volume])
		
	def set_music_volume(self, new_volume):
		if not self.enabled: return
		base.messenger.send("set_music_volume", [new_volume])
