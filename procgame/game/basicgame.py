from . import GameController, PinboxGameController
...
class BasicPinboxGame(PinboxGameController):
	""":class:`BasicGame` is a subclass of :class:`~procgame.game.GameController` 
	that includes and configures various useful helper classes to provide:
	...
	* A :class:`~procgame.desktop.Desktop` helper at ``self.desktop`` configured 
	...
	def __init__(self, machine_type):
		super(BasicPinboxGame, self).__init__(machine_type)
		#self.score_display = ScoreDisplay(self, 0)
		
		self.keyboard = base.keyboard


	def load_config(self, path):
		super(BasicPinboxGame,self).load_config(path)
		...
		if self.keyboard:
			for k, v in key_map_config.items():
				...
				if self.switches.has_key(switch_name):
					...
					switch_number = pinproc.decode(self.machine_type, switch_name)
				self.keyboard.add_key_map(str(k), switch_number)

	def reset(self):
		"""Calls super's reset and adds the :class:`ScoreDisplay` mode to the mode queue."""
		super(BasicPinboxGame, self).reset()
	...
	def get_events(self):
		"""Overriding GameController's implementation in order to append keyboard events."""
		events = super(BasicPinboxGame, self).get_events()
		if self.keyboard: events.extend(self.keyboard.get_keyboard_events())
		return events
	...
	def tick(self):
		...
		Displays the last-received DMD frame on the desktop."""
		super(BasicPinboxGame, self).tick()
	...
	def score(self, points):
		...
		# Dispatch an event to our display subsystem so the scores are updated on screen
